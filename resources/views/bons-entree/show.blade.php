        ...
        <div class="col-md-6">
            <div class="card mb-4">
	            <div class="card-header text-light bg-info d-flex justify-content-between">
	            	...
	            	<div class="d-flex">
	            		@can('viewHistory', $bon)
	            			<a href="/bon-entree/{{ $bon->id }}/history" class="pr-2"><img src="/images/history.png" style="height: 16px; width: 16px;" alt="historique"></a>
	            		@endcan
	            		@can('update', $bon)
	            		...
	            		@endcan
	            		@can('delete', $bon)
		            		<form action="/bon-entree/{{ $bon->id }}" method="post">
		            			@csrf
	                    		@method('DELETE')
				...
				<div class="card-body">
					<span><strong>Date d'entrée : </strong>{{ \Carbon\Carbon::parse($bon->date)->translatedFormat('d F Y à H\hi') }}</span>
					<hr>
