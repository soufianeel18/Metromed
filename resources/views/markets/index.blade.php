    ...
    <div class="row">
        <div class="col-md-12">
            <div class="d-flex justify-content-center pb-3">
                <h1>Liste des clients/marché</h1>
            </div>
            @can('create', \App\Market::class)
                <div class="d-flex justify-content-between">
                    <div></div>
                    <div>
                        <button class="btn btn-outline-danger" onclick="window.location.href='/market/create'">Ajouter un marché</button>
                    </div>
                </div>
                <hr>
            @endcan
                ...
                @foreach($markets as $market)
                    <div class="d-flex align-items-center justify-content-between pb-3">
                        <div class="d-flex align-items-center">
                            <div class="h3 mr-3">{{ $market->ville }} {{ $market->n_market }}</div>
                            @can('view', $market)
                                <a href="/market/{{ $market->id }}" class="mr-3">
                                    <img src="/images/history.png" style="height: 20px; width: 20px;" alt="historique">
                                </a>
                            @endcan
                            @can('update', $market)
                                <a href="/market/{{ $market->id }}/edit" class="mr-3">
                                    <img src="/images/edit.png" style="height: 20px; width: 20px;" alt="modifier">
                                </a>
                            @endcan
                            @can('delete', $market)
                                <form action="/market/{{ $market->id }}" method="post" class="mr-3">
                                    @csrf
                                    @method('DELETE')
                                    <button style="border: none; padding: 0px; background-color: transparent;"><img src="/images/delete.svg" style="height: 20px; width: 20px;" alt="supprimer"></button>
                                </form>
                            @endcan
                        </div>
                        <div>
                            @can('create', \App\Client::class)
                                <button class="btn btn-outline-info" onclick="window.location.href='/market/{{ $market->id }}/client/create'">Ajouter un client</button>
                            @endcan
                        </div>
                    </div>
                    @if($market->clients->count() == 0)
                        <div class="d-flex justify-content-center">
                            <span>Pas de données</span>
                        </div>
                    @else
